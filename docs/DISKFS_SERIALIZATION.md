...
snapshots, descriptor offsets or caller user buffers into transactions. Indexed descriptor
close/reuse and runtime standard-stream I/O/dup2/fork now have syscall-layer
gates in [descriptor serialization](DESCRIPTOR_SERIALIZATION.md). User-buffer
lifetime during VFS/device sleep and initializing shell redirection before
task publication remain prerequisites for a general blocking-I/O rollout.
The gate is single-core and does not promise strict fairness or SMP support.

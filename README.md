...
- **User land:** 51 system calls, ~37 user programs/demos, a kernel-space shell
...
**`/proc`**;per-process 環境變數;RTC 牆鐘時間與 CPU 時間計量;47 個系統呼叫、
約 37 支使用者程式、核心 shell 與 **ring-3 使用者 shell(`ush`)**;以及一套
確定性的自動化 `make test`。

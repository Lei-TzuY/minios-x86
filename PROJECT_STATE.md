...
- **BUILD1（2026-09-20）**：核心 C 物件新增編譯器產生的標頭依賴，修正只改
  `task.h` 卻沿用舊 `timer.o`／`kernel.bin` 的增量建置錯誤。直接、間接標頭、
  Makefile 更新、舊物件遷移、移除標頭與 clean 都由真實核心建置回歸守護；
  增量核心須與乾淨重建位元組相同。命令列旗標／工具鏈變更仍需 clean。
